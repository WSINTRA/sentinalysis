...
static SSH_ACCEPTED_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"Accepted (?:password|publickey) for (?P<user>\S+) from (?P<ip>\S+) port \d+")
        .expect("hardcoded SSH accepted login regex must be valid")
});
