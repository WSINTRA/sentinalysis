...
impl NoiseFilter {
    ...
    pub fn new() -> Self {
        Self {
            ...
            static_asset_patterns: Arc::new(
                Regex::new(r"\.(css|js|png|jpg|jpeg|gif|ico|svg|woff|woff2|ttf|eot)$").unwrap(),
            ),
            bot_user_agents: vec![
                Arc::new(Regex::new(r"Googlebot").unwrap()),
                Arc::new(Regex::new(r"Bingbot").unwrap()),
                Arc::new(Regex::new(r"YandexBot").unwrap()),
                Arc::new(Regex::new(r"Slurp").unwrap()),
                Arc::new(Regex::new(r"DuckDuckBot").unwrap()),
            ],
    ...
    pub fn evaluate(&self, entry: &ParsedLogEntry) -> FilterResult {
        ...
        if let Some(ip) = &entry.metadata.client_ip && self.excluded_ips.contains(ip) {
