...
use notify::{Config, Event, EventKind, EventHandler, RecommendedWatcher, RecursiveMode, Watcher};
...
mod tests {
    ...
    async fn test_tailer_reads_existing_content() {
        let mut file = create_test_file("line1\nline2\nline3");
    ...
    async fn test_tailer_detects_new_lines() {
        ...
        tailer.stop();
        assert!(!new_lines.is_empty(), "Expected to detect new line via file watching");
    }

    #[tokio::test]
    async fn test_tailer_tracks_byte_offset() {
        let mut file = create_test_file("first line");
        let path = file.path().to_path_buf();
