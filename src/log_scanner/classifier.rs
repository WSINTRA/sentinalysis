...
mod checks {
    ...
    use crate::log_scanner::parser::{LogMetadata, LogLevel};
    ...
    pub fn check_command_injection(result: &mut ThreatResult, text: &str, path: &str) {
        let patterns = [
            ...
            "$(wget", "$(curl", "$(cat ", ";wget ", ";curl ", ";nc ", ";bash ", ";sh ",
    ...
    pub fn check_sql_injection(result: &mut ThreatResult, text: &str, path: &str) {
        let patterns = [
            "' or '", "' or 1", " or 1=1", " or '1'='1", "union select", "union all select",
            "drop table", "drop database", "insert into", "delete from", "update .* set",
            "exec xp_", "exec sp_", "';--", "'--", "admin'--", "1; drop", "1 and 1=1",
            "1' and '1'='1",
        ];
    ...
    pub fn check_xss(result: &mut ThreatResult, text: &str, path: &str) {
        let patterns = [
            ...
            "<img src=", "<svg ", "<iframe", "alert(", "document.cookie", "eval(",
        ];
    ...
    pub fn check_path_traversal(result: &mut ThreatResult, path: &str) {
        let patterns = [
            "../", "..\\", "..%2f", "..%5c", "%2e%2e/", "....//", "/etc/passwd", "/etc/shadow",
            "/proc/self", "php://filter", "php://input", "file://",
        ];
    ...
    pub fn check_brute_force(result: &mut ThreatResult, text: &str, entry: &ParsedLogEntry) {
        let patterns = [
            ...
            "maximum authentication attempts", "too many authentication failures",
    ...
    pub fn check_scanner_ua(result: &mut ThreatResult, metadata: &LogMetadata) {
        ...
        let scanners = [
            "sqlmap", "nikto", "nmap", "masscan", "hydra", "medusa", "burp suite",
            "dirbuster", "gobuster", "wpscan", "nuclei", "ffuf", "feroxbuster", "w3af",
        ];
    ...
    pub fn check_sensitive_files(result: &mut ThreatResult, path: &str) {
        let patterns = [
            "/.env", "/.git", "/.htaccess", "/.htpasswd", "/wp-config.php",
            "/config/database.yml", "/config/database.yaml", "/config/secrets.yml",
            "/.aws/credentials", "/.ssh/id_rsa", "/proc/self/environ", "/boot.ini",
            "/web.config", "/phpinfo.php", "/server-status", "/.svn", "/.DS_Store",
        ];
    ...
    pub fn check_tls_errors(result: &mut ThreatResult, text: &str) {
        let patterns = [
            "ssl_do_handshake", "ssl_error", "certificate verify failed", "tls alert",
            "handshake failure",
        ];
    ...
    pub fn check_error_responses(result: &mut ThreatResult, metadata: &LogMetadata, path: &str) {
        ...
        if status == 403 || status == 404 {
            let suspicious_paths = [
                "/admin", "/wp-admin", "/phpmyadmin", "/manager", "/console", "/config",
                "/.env", "/backup",
            ];
    ...
    pub fn calculate_threat_level(result: &ThreatResult) -> ThreatLevel {
        let has_critical = result.categories.contains(&ThreatCategory::CommandInjection);
        ...
        let has_medium = result.categories.iter().any(|c| {
            matches!(c, ThreatCategory::BruteForce | ThreatCategory::SensitiveFile)
        });
...
mod tests {
    use super::*;
    use crate::log_scanner::parser::{LogMetadata, LogLevel, ParsedLogEntry};
    use chrono::Utc;
    use rstest::rstest;
    use std::net::IpAddr;
    ...
    fn test_classify_brute_force_indicators() {
        ...
        let cases = vec![
            ("Failed password for root from 192.168.1.100", LogLevel::Error),
    ...
    fn test_classify_command_injection() {
        ...
        for path in cases {
            ...
            assert!(
                result.categories.contains(&ThreatCategory::CommandInjection),
    ...
    fn test_threat_level_ordering(
        #[values(ThreatLevel::None, ThreatLevel::Low, ThreatLevel::Medium, ThreatLevel::High, ThreatLevel::Critical)] a: ThreatLevel,
        #[values(ThreatLevel::None, ThreatLevel::Low, ThreatLevel::Medium, ThreatLevel::High, ThreatLevel::Critical)] b: ThreatLevel,
    ) {
